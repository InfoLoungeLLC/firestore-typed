...
- **Dependencies**: Added `firebase-admin` to devDependencies to work around npm CLI issue with optional peer dependencies
  - npm CLI has a known issue where optional peer dependencies listed in devDependencies are not installed during `npm ci` ([npm/cli#8464](https://github.com/npm/cli/issues/8464))
  - This was causing test failures in GitHub Actions CI environments
  - All peer dependencies (firebase-admin, typia, zod) are now properly duplicated in devDependencies for development and testing

## [0.5.3] - 2025-08-11

### Changed

- **Dependencies**: Updated peer dependencies to match dev dependencies for consistency
  - typia: ^9.6.1 → ^9.7.1
  - zod: ^4.0.14 → ^4.0.17
